...
- [~] **B2. Tizen TV** — **packaging done; only the install target is missing.**
  Tizen Studio is EOL, so the toolchain is now the **Tizen VS Code extension** and
  its `tizen-core` CLI (`tz`). Done: author certificate created *locally* with
  `tz cert` (no Samsung account — public distributor cert is bundled), signing
  profile added, and `pnpm package:tizen` produces a **verified signed `.wgt`**
  (37 KB, correct manifest + both signatures). The missing app `icon.png` that
  would have broken packaging on Tizen *and* webOS is now generated by
  `pnpm icon`.
  *Remaining:* a **TV emulator image** (Package Manager, needs elevation — *Tizen:
  Package Manager* in VS Code) or a **Samsung TV in Developer Mode**; then
  `sdb connect` → `tz install` → `?diag` + acceptance script. All flags (`?diag`,
  `?llm=`, `?confirm=`) already work in the Tizen entry.
